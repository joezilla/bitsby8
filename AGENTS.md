...
- `npm run dev -- <args>`: Run TypeScript directly with ts-node for quick iteration.
- `npm run build`: Type-check and emit JS + maps into `dist/`.
- `npm start -- <args>`: Execute the compiled server from `dist/`.
- `npm test`: Run Jest (ts-jest preset) against `test/**/*.test.ts`; outputs coverage to `coverage/`.
...
- Avoid editing `dist/` directly; regenerate via `npm run build`. Keep config path defaults (`DEFAULT_CONFIG_LOCATIONS`) in sync with CLI help output.
...
- Add coverage for new modules; run `npm test -- --runInBand` if serializing hardware-mock tests locally.
...
- Before opening a PR: `npm run build` and `npm test` should pass; include any Makefile-driven packaging steps if the change affects Debian artifacts.
